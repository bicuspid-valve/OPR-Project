"""Focused probe: K=30 (argmax + 29 random), 100 games.

Pushing K much higher than the previous K=9 probe. Question: does V
keep finding improvements as more random candidates are available, or
does the per-candidate marginal value diminish/plateau?

K=9 reading was 0.582 on decisive games (149/256, p≈0.008). If V's
useful resolution is essentially binary ("argmax is best vs argmax
isn't"), the decisive-game advantage shouldn't grow much with K.
If V can rank actions on a finer scale, more candidates means a
better V-best-pick, and the asymmetry should grow.
"""
import json
import math
import random
...
def main():
    ...
    print(f"K={K} (1 argmax + {K-1} random), games={NUM_GAMES}, workers={NUM_WORKERS}")
    print()
    ...
    with mp.Pool(NUM_WORKERS, initializer=_worker_init, initargs=(ckpt, hof)) as pool:
        ...
        for r in pool.imap_unordered(_play, range(NUM_GAMES)):
            ...
            if done % 10 == 0 or done == NUM_GAMES:
                p = wins["planner"] / done
    ...
    print(f"planner_wr (incl. draws) = {p:.4f} ± {se:.4f}  vs 0.500: z = {z_vs_50:+.2f}")
    ...
    if decisive > 0:
        ...
        print(f"Decisive games (n={decisive}): planner_wr = {p_dec:.4f} ± {se_dec:.4f}  z={z_dec:+.2f}")
